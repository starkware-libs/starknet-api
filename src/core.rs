...
pub const L2_ADDRESS_UPPER_BOUND: Lazy<FieldElement> = Lazy::new(|| {
    FieldElement::from_hex_be(PATRICIA_KEY_UPPER_BOUND).unwrap() - MAX_STORAGE_ITEM_SIZE.into()
});
...
pub fn calculate_contract_address(
    ...
    let raw_address = pedersen_hash_array(&[
        StarkFelt::try_from(contract_address_prefix_hex.as_str()).unwrap(),
