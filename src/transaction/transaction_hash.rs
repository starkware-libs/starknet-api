...
static DECLARE: Lazy<StarkFelt> =
    Lazy::new(|| ascii_as_felt("declare").expect("should be a valid ascii"));
static DEPLOY: Lazy<StarkFelt> =
    Lazy::new(|| ascii_as_felt("deploy").expect("should be a valid ascii"));
static DEPLOY_ACCOUNT: Lazy<StarkFelt> =
    Lazy::new(|| ascii_as_felt("deploy_account").expect("should be a valid ascii"));
static INVOKE: Lazy<StarkFelt> =
    Lazy::new(|| ascii_as_felt("invoke").expect("should be a valid ascii"));
static L1_HANDLER: Lazy<StarkFelt> =
    Lazy::new(|| ascii_as_felt("l1_handler").expect("should be a valid ascii"));
...
static CONSTRUCTOR_ENTRY_POINT_SELECTOR: Lazy<StarkFelt> = Lazy::new(|| {
    StarkFelt::try_from("0x28ffe4ff0f226a9107253e17a904099aa4f63a02a5621de0576e5aa71bc5194")
        .expect("should be a valid felt")
});
static TWO: Lazy<StarkFelt> = Lazy::new(|| StarkFelt::from(2_u8));

/// Calculates hash of a Starknet transaction.
/// See transaction types [documentation](https://docs.starknet.io/documentation/architecture_and_concepts/Network_Architecture/transactions) for more details.
...
///
/// # NOTE
/// A hash is considered valid if it is the result of one of the hash functions that were ever used
/// in Starknet.
...
fn get_common_deploy_transaction_hash(
    ...
    Ok(TransactionHash(
        ...
        .chain_if(&StarkFelt::ZERO, !is_deprecated) // No fee in deploy transaction.
        .chain(&ascii_as_felt(chain_id.0.as_str())?)
        .get_hash(),
    ))
}
...
fn get_common_invoke_transaction_v0_hash(
    ...
    Ok(TransactionHash(
        PedersenHashChain::new()
        .chain(&INVOKE)
        .chain_if(&StarkFelt::ZERO, !is_deprecated) // Version
...
fn get_common_l1_handler_transaction_hash(
    ...
    Ok(TransactionHash(
        ...
        .chain(&transaction.entry_point_selector.0)
        .chain(&PedersenHashChain::new().chain_iter(transaction.calldata.0.iter()).get_hash())
        .chain_if(&StarkFelt::ZERO, version > L1HandlerVersions::V0Deprecated) // No fee in l1 handler transaction.
        .chain(&ascii_as_felt(chain_id.0.as_str())?)
