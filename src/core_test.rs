...
fn patricia_key_macro() {
    assert_eq!(patricia_key!(0x123), PatriciaKey::try_from(Felt::from(0x123)).unwrap());
}
...
fn test_calculate_contract_address() {
    ...
    let address = Pedersen::hash_array(&[
        ...
        deployer_address.0.to_felt(),
        salt.0,
        class_hash.0,
        constructor_calldata_hash,
    ]);
    let mod_address: Felt =
        address.mod_floor(&(TryInto::<NonZeroFelt>::try_into(*L2_ADDRESS_UPPER_BOUND).unwrap()));
...
fn eth_address_serde() {
    let eth_address = EthAddress::try_from(Felt::from(0x001)).unwrap();
