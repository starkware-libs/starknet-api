...
pub fn calculate_receipt_commitment<H: HashFunction>(
    ...
    l1_gas_wei_price: GasPrice,
) -> ReceiptCommitment {
    ReceiptCommitment(calculate_root::<H>(
        transactions_receipt
            .iter()
            .map(|receipt| calculate_receipt_hash(receipt, l1_data_gas_wei_price, l1_gas_wei_price))
            .collect(),
    ))
}
...
fn chain_execution_resources(
    ...
    let l1_gas_consumed: u128 = (actual_fee.0
        - l1_data_gas_wei_price.0 * u128::from(execution_resources.da_l1_data_gas_consumed))
        / l1_gas_wei_price.0;
    hash_chain
